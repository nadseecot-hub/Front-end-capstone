...
const stats = [
  ["Profile Views", 1248, "+12.5%", "views"],
  ["Saves", 342, "+8.2%", "saves"],
  ["Pending Requests", 8, "Needs attention", "requests"],
  ["Unread Messages", 5, "2 new today", "messages"],
] as const;

const bookings = {
  Pending: [{ name: "Maya Chen", subject: "Algebra", time: "Today · 4:00 PM", avatar: "MC" }, { name: "Oliver Grant", subject: "Calculus", time: "Tomorrow · 10:30 AM", avatar: "OG" }],
  Upcoming: [{ name: "Sofia Patel", subject: "Physics", time: "Thu · 3:30 PM", avatar: "SP" }, { name: "Ethan Brooks", subject: "Mathematics", time: "Fri · 5:00 PM", avatar: "EB" }],
  Past: [{ name: "Nora Williams", subject: "Algebra", time: "Aug 22 · Completed", avatar: "NW" }, { name: "Liam Scott", subject: "Physics", time: "Aug 19 · Completed", avatar: "LS" }],
} satisfies Record<BookingTab, { name: string; subject: string; time: string; avatar: string }[]>;

const activity = [
  ["New booking request", "Maya Chen requested an Algebra session", "12 min ago", "calendar"],
  ["Profile saved", "Your profile was saved by Oliver Grant", "2 hours ago", "heart"],
  ["New message", "Sofia sent you a message", "Yesterday", "message"],
  ["Session completed", "You completed a Physics session with Liam", "Aug 19", "check"],
] as const;
...
export default function DashboardPage() {
  ...
  return <div className="dashboard-page">
    ...
    <main className="dashboard-main">
      <header className="dashboard-header dashboard-reveal"><div><p className="dashboard-kicker">TUESDAY, AUGUST 27, 2026</p><h1>Good morning, Alex <span>✦</span></h1><p>Here’s what’s happening with your tutoring profile.</p></div><DashboardHeaderActions /></header>
      ...
      <section className="dashboard-grid dashboard-reveal" style={{ "--delay": "280ms" } as React.CSSProperties}>
        ...
        <article className="dashboard-card dashboard-completeness"><div className="dashboard-card-heading"><div><h2>Profile Completeness</h2><p>A complete profile gets more bookings.</p></div><span className="dashboard-card-more">•••</span></div><div className="dashboard-completeness__body"><div className="dashboard-progress"><svg viewBox="0 0 120 120" width="120" height="120" role="img" aria-label="Profile is 80 percent complete"><circle className="dashboard-progress__track" cx="60" cy="60" r="50" /><circle className="dashboard-progress__value" cx="60" cy="60" r="50" /></svg><strong><CountUp value={80} />%</strong></div><div><strong>You’re almost there!</strong><p>Add your availability and a profile video to reach 100%.</p><Link href="/dashboard/profile" className="dashboard-text-link">Complete Profile <span>→</span></Link></div></div></article>
      </section>
